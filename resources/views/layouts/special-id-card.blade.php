...
<html lang="en" >
	...
	<body id="kt_body" class="header-fixed header-mobile-fixed page-loading">
		...
		<script src="{!! asset('js/specialentity.js?v=1.0.1') !!}" type="text/javascript" ></script>
		...
		@php
			...
			if (isset($entityApplicationDetailData)) {
				if(!empty($entityApplicationDetailData) && $entityApplicationDetailData->mobile_number){
					echo "<script>set_country_code('special_entity_application_mobile_number','".strtolower($entityApplicationDetailData->country_code)."')</script>";
				}
			}
			else {
				echo "<script>set_country_code('special_entity_application_mobile_number','in')</script>";
			}
		@endphp
        @yield('scripts')
		<!--end::Page Scripts-->
